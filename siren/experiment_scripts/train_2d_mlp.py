...
import torch
from vqvae_latents_dataset import LatentsDataset
...
from siren import dataio_2d, loss_functions, sdf_meshing, training, utils
...
def main(cfg):
    ...
    for i, file in enumerate(files):
        ...
        for j in range(1):
            ...
            latents_dataset = dataio_2d.VQVAELatents(
                os.path.join(cfg.dataset_folder, file),
            )
            dataloader = DataLoader(
                latents_dataset, shuffle=True, batch_size=1, pin_memory=True, num_workers=0
            )
            ...
            if i < 5:
                ...
                if cfg.mlp_config.move:
                    imgs = []
                    for time_val in range(latents_dataset.total_time):
